...
export const isWyreAllowedToBuy = (chainId) =>
  ...
  (chainId === NETWORKS_CHAIN_ID.KOVAN && Device.isIos());
...
const getMerchantIdentifier = (network) =>
  network === '1' ? WYRE_MERCHANT_ID : WYRE_MERCHANT_ID_TEST;
const getPartnerId = (network) =>
  network === '1' ? WYRE_ACCOUNT_ID : WYRE_ACCOUNT_ID_TEST;
...
const getRates = (network) =>
  getWyreAPI(network).get(`v3/rates`, { params: { as: 'PRICED' } });
...
const getOrderStatus = (network, orderId) =>
  getWyreAPI(network).get(`v3/orders/${orderId}`);
const getTransferStatus = (network, transferId) =>
  getWyreAPI(network).get(`v2/transfer/${transferId}/track`);
export const getOrderQuotation = (
  network,
  { amount, currency, address, country = 'US' },
  cancelToken,
) =>
...
const destToAddress = (dest) =>
  dest.indexOf('ethereum:') === 0 ? dest.substring(9) : dest;
...
const wyreTransferToFiatOrder = (wyreTransfer) => ({
  ...
  cryptoFee: wyreTransfer.fees
    ? wyreTransfer.fees[wyreTransfer.destCurrency]
    : null,
  ...
  try {
    ...
    if (transferId) {
      try {
        ...
        Logger.error(error, {
          message:
            'FiatOrders::WyreApplePayProcessor error while processing transfer',
          order,
        });
      }
    }
    ...
    Logger.error(error, {
      message: 'FiatOrders::WyreApplePayProcessor error while processing order',
      order,
    });
    return order;
  }
  ...
  displayItems: [
    {
      ...
      label: strings('fiat_on_ramp.wyre_purchase', {
...
export function useCountryCurrency(country) {
  const currency = useMemo(
    () => SUPPORTED_COUNTRIES[country]?.currency,
    [country],
  );
  return {
    ...
    async function getWyreRates() {
      try {
        ...
        currencies.forEach((pair) => {
          if (
            pair.length % 2 === 0 &&
            pair.slice(0, pair.length / 2) === pair.slice(pair.length / 2)
          ) {
        ...
        Logger.error(
          error,
          'FiatOrders::WyreAppleyPay error while fetching wyre rates',
        );
      }
    }
    getWyreRates();
...
export function useWyreOrderQuotation(
  network,
  amount,
  currency,
  address,
  ...
  const showRequest = useCallback(
    async (amount, fee, decimals) => {
      ...
      try {
        ...
        Logger.error(error, {
          message: 'FiatOrders::WyreApplePayPayment error while creating order',
        });
        throw error;
      }
    },
